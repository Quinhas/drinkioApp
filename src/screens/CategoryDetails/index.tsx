...
import { Box, Button, Icon, Image, Skeleton, Text } from 'native-base';
...
export function CategoryDetails() {
  ...
  async function getCategoryDetails() {
    try {
      ...
      const data = await drinkioApi.getCategoryDetails({ id });
      setCategory(data);
    } catch (error) {
    } finally {
      setIsLoading(false);
    ...
    return (
      <CustomView>
        <Skeleton
          ...
          shadow={4}
        />
        <DrinksList isLoading={isLoading} drinks={[]} />
      </CustomView>
    );
  }
        ...
        <Box w='100%' h='100%' zIndex={10} position='absolute'>
          ...
          >
            <Icon
              as={FontAwesome}
              name={
                favoritesCategories?.find((value) =>
                  value === category.id)
                  ? 'star'
                  : 'star-o'
              }
