...
export function DrinksList({ isLoading, drinks }: DrinksListProps) {
    ...
    return (
      <Flex justify={"center"} align={"center"} mt={'1.5rem'}>
        <Icon
          ...
          color={"muted.500"}
        />
        <Text fontWeight={"medium"} color={"muted.500"} fontSize={"1.25rem"} mt={'1rem'}>
  ...
  return (
    <VirtualizedList<DrinkProps>
      data={drinks}
      renderItem={({ item }) => <DrinkCard key={item.id} {...item} />}
      keyExtractor={(item) => String(item.id)}
      getItemCount={() => drinks.length}
      getItem={(data, index) => drinks[index]}
    />
  );
}
